...
namespace AnimalSitter.Common
{
    internal class DialogueManager
    {
        ...
        ** Public methods
        *********/
        public DialogueManager(IConfig config, IModContentHelper content, IMonitor monitor)
        {
            Config = config;
            Content = content;
            Monitor = monitor;
        ...
        public string PerformReplacement(string message, IStats stats, IConfig config)
        {
            ...
            fields["spouse"] = Game1.player.isMarriedOrRoommates() ? Game1.player.getSpouse().getName() : config.WhoChecks;
        ...
        public void ReadInMessages()
        {
            ...
            try
            {
                AllMessages = Content.Load<Dictionary<string, string>>("assets/dialog.xnb");
            }
            catch (Exception ex)
            {
                Monitor.Log($"[oliver-StardewLib] Exception loading content:{ex}", LogLevel.Error);
            }
        }
